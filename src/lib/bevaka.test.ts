...
describe('matchesBevakning', () => {
  it('matches when an offer name contains a sok keyword', () => {
    expect(matchesBevakning(makeItem({ sok: ['gevalia'] }), { ...makeOffer(), store: 'willys', week: '2026-W31' })).toBe(true)
  })

  it('matches against the brand field too, not just namn', () => {
    const item = makeItem({ sok: ['zoegas'] })
    const offer = { ...makeOffer({ namn: 'Bryggkaffe', marke: 'Zoégas' }), store: 'willys', week: '2026-W31' }
  ...
  it('does not match when no sok keyword is present', () => {
    ...
    const offer = { ...makeOffer({ namn: 'Gevalia Kaffe', marke: 'Gevalia' }), store: 'willys', week: '2026-W31' }
  ...
  it('undvik_marken does not disqualify a different brand', () => {
    const item = makeItem({ sok: ['kaffe'], undvik_marken: ['gevalia'] })
    const offer = { ...makeOffer({ namn: 'Löfbergs Kaffe', marke: 'Löfbergs' }), store: 'willys', week: '2026-W31' }
  ...
  it('an empty sok watches the whole category (matched by group, not leaf)', () => {
    ...
    const fruitOffer = { ...makeOffer({ namn: 'Bananer', kategori: 'frukt' }), store: 'willys', week: '2026-W31' }
    const otherOffer = { ...makeOffer({ namn: 'Mjölk', kategori: 'mjolk_gradde' }), store: 'willys', week: '2026-W31' }
  ...
  it('an empty sok still respects undvik_marken', () => {
    ...
    const offer = { ...makeOffer({ namn: 'Bananer', marke: 'Chiquita', kategori: 'frukt' }), store: 'willys', week: '2026-W31' }
    expect(matchesBevakning(item, offer)).toBe(false)
  })
})
...
describe('toOfferRef', () => {
  it('extracts just {store, week} from a tagged offer', () => {
    const tagged = { ...makeOffer(), store: 'ica', week: '2026-W29' }
...
describe('findBevakaHits', () => {
  it('only returns items that have at least one matching offer', () => {
    ...
    const offers = [{ ...makeOffer({ namn: 'Gevalia Kaffe' }), store: 'willys', week: '2026-W31' }]
