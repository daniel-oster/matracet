...
**Färdigmat bug: pizza-***making*** components landed in "ready meals" (fixed 2026-07)**: user-reported — after the split above, `fardigmat` was "basically all half fabricates" instead of frozen dinners. Cause: `FARDIGMAT_RE`'s bare `pizza`/`glass` keywords matched not just finished frozen pizza/ice cream but also pizza-*making* components — `Pizzabotten` (dough base), `Pizzamjöl` (flour), `Pizzasås` (sauce), `Pizza topping`, `Pizzakit` (build-your-own kit), `Pizzamix Riven 28%` (a shredded-cheese blend), and `Glassås` (chocolate/caramel sauce *for* ice cream, not ice cream itself) — none of which are a heat-and-eat dish. Fixed with negative lookaheads: `pizza(?!\s*(?:botten|mjöl|sås|kit|mix|topping))` and `glass(?!ås)`, so these fall through to the categories they actually belong in instead — `Pizzamjöl`/`Pizzasås`/`Glassås` land in `skafferi` via its pre-existing `mjöl(?!k)`/`sås` keywords with zero new additions needed, `Pizzabotten`/`Pizzakit`/`topping` needed new explicit `skafferi` keywords since nothing else matched them, and `Pizzamix` (the cheese blend) got its own `mejeri` keyword. Re-ran the same narrow, safe-by-construction migration pattern scoped to just `kategori: 'fardigmat'` this time (not all of `ovrigt` again) — 9 of 57 `fardigmat` offers were reclassified (8 to `skafferi`, 1 to `mejeri`), leaving `fardigmat` as a clean 28 unique items, all genuinely frozen/chilled ready-to-eat product (pizza, glass, gratäng, paj, pommes, soppa, enportionsrätter) with no assembly-required ingredients left in it. Worth remembering for any future category: a bare product-family keyword (`pizza`, `glass`, `kaffe`, ...) can match both the finished product *and* its raw components/accessories sold under the same word — check for that split explicitly, not just for cross-category collisions with *other* keywords.
