...
- **Never set a recipe's `bildUrl` from a guessed stock-photo id — download it and actually look at it first.** The citronbrownies recipe shipped with an Unsplash id picked purely because the slug sounded lemon-adjacent; it turned out to be a *lemon meringue pie*, and the household noticed before anyone here did. `curl` the URL to a file and open it with the `Read` tool — it costs one tool call and is the only thing that actually verifies the picture matches the dish. Related gotcha found in the same pass: **`bildUrl` has to be set in two places** — `recept.json` (drives `RecipeOverlay`'s hero image *and* the `og:image` on the generated `dist/recept/<slug>/` share page) and the recipe's `_index.json` entry (drives the card thumbnail in `ReceptView`/`Hub`). Setting only the index leaves the overlay with no hero at all, which is easy to miss because the card looks fine. **Check the source blog's own terms before copying its photo into the repo**: landleyskok.se states outright at the bottom of every recipe that its images are upphovsrättsskyddade and may not be reused elsewhere, so a recipe imported from there gets a verified stock photo instead — the committed-`bild.jpg` route (next lesson) is only for sources that actually permit it.
- **A recipe image may live in the repo instead of on a remote host — put it at `public/data/recipes/<slug>/bild.jpg` and set `bildUrl` to `/matracet/data/recipes/<slug>/bild.jpg`.** Most recipes hotlink Unsplash, but citronbrownies uses the source recipe's own photo, and a hotlink to a food blog can break the day they reorganise their uploads; a committed copy can't. Two things this needs that a remote URL doesn't: `scripts/build-recipe-pages.mjs` absolutizes `og:image` via `new URL(bildUrl, SITE_URL).href` (a crawler fetches the share card out of context and will *not* resolve a site-relative path — an already-absolute URL passes through the same line untouched, so both kinds of `bildUrl` work), and it's the one case where the image genuinely *can* be verified in-app with Playwright, since it's same-origin (see the next lesson). Keep an eye on repo size if this becomes the norm — the full-resolution original here is 128 KB, fine once, less fine × 130 recipes; there's no image-resizing tool in the sandbox (no PIL, no ImageMagick), so prefer a smaller variant from the source when one exists.
- **The sandbox's headless browser cannot load *any* remote image, so a recipe photo can never be verified through a Playwright screenshot here.** `naturalWidth` comes back `0` for `images.unsplash.com` just as much as for a new host — this is the sandbox's outbound network, not a bad URL, so don't go debugging the URL. Two-step workaround that does work: (1) verify the *file* by `curl`-ing it and viewing it, and (2) verify the *crop/layout* by copying the image into `public/` temporarily, pointing `bildUrl` at `/matracet/<file>`, screenshotting the card and overlay, then restoring the real URL and deleting the temp file. Worth doing — a portrait photo behind `.overlay-hero img`'s `max-height: 260px; object-fit: cover` is centre-cropped, and whether that lands on the food or on the garnish is not something you can reason out from the JSON.
...
- **A store's HTML markup drifts, and a parser keyed to a styled-components hash class fails *silently and totally*.** The 2026-W33 Hemköp import parsed **0 offers out of 67 intact product blocks**: `erbjudanden-parse-hemkop-html.mjs` read the price from `class="sc-7d7c3adf-1..."`, that hash changed on some Hemköp redeploy, and the price-block match returned null — which the parser treats as "not an offer card" and skips. Nothing errored; the run just reported a suspiciously round zero. The same file's header had *already* warned (about the Willys parser) to key off stable `itemprop`/`data-testid` markers and never the volatile `sc-xxxx` classes — the advice was written down and then not followed in this parser. Fixed by reading the price from `data-testid="screen-reader-text"` (the card's own accessible price string, e.g. `"15,00 /st"`, `"2 för 28 kr"`) and klubbpris from an explicit `<p>Klubbpris</p>` label. **Two sub-lessons worth keeping**: (1) the splash *colour* testid (`splash-yellow`/`splash-red`/`pricebomb-splash`) looks like it encodes klubbpris and does not — 39 of 41 yellow cards had the Klubbpris label but two multi-buy cards did not, so match the label, never the colour; (2) an offer count of exactly 0 (or a big drop) is the only symptom this class of bug produces — compare against the previous week's count on every import before trusting the output, since a partial break would be even quieter.
- **A stable class name is not a stable *selector*: a card can grow an extra attribute and break a regex that expected `class="x">`.** The 2026-W37 ICA import parsed 116 of 138 offer cards; the 22 missing ones rendered their title as `<p class="offer-card__title" data-v-2aa9b146="">` (a Vue scoped-style attribute) while the rest had no such attribute, and `offer-card__title">` only matched the latter. Same silent-total-failure shape as the W33 Hemköp hash-class break, one level subtler — the parser *was* keyed to a stable class token, it just also pinned the attribute's exact position. Every class/testid lookup in these parsers now allows attributes before the tag closes (`offer-card__title"[^>]*>`); write new ones that way from the start. Two related things found in the same import: Hemköp's "Bara för dig" coupons had moved into a **third** card shape (no `product-main-link`, no `product-container` — a plain card above the `offline-promotion-products` grid), so the parser now treats *any* `product-title` no container range covers as its own card and feeds both shapes through one code path; and Hemköp's **last** `product-container` was sliced with a fixed 6000-char fallback window that its 9042-char card outran, silently dropping exactly one offer — the final container now runs to end-of-document.
- **ICA's per-offer end date is in the page state blob, not the card.** `"id":"<promotion id>","details":{…},"validTo":"2026-09-13T00:00:00"` keys off the same id the card carries as `data-promotion-id`, and ICA staggers them hard: in 2026-W37, 89 of 138 cards ended with the flyer week and 49 ran one to three weeks longer (they were overwhelmingly the non-grocery campaign items — only 2 survived the non-food filter). `erbjudanden-parse-ica-html.mjs` now reads it and applies the same majority-is-the-file's-date convention the Hemköp parser uses. Willys' page exposes no per-offer date at all (checked, not assumed).
- **A classifier fix does not reach a product the lexicon has already cached — audit that explicitly on every import.** "Kryddkorvar | GARANT" (a sausage) had been filed as `kryddor_buljong` since 2026-W25 because `krydd` won over a `korv\b` that the Swedish plural defeated. The `korv(?:ar|en)?\b` fix landed weeks ago and changed nothing for this product: `erbjudanden-recategorize.mjs` is lexicon-first, so the stale `regel` verdict kept replaying. The cheap check, worth running once per import — for every offer in the week's files, compare its lexicon verdict against a fresh `classify()` call and eyeball the disagreements. Most will be the lexicon being *better* than the rules (a reviewed `model`/`manuell` entry), which is the point of it; a rules-are-better disagreement is a stale cache, and the fix is to correct those specific keys (never a blanket re-derive, and never touching already-committed past weeks).

- **A sixth substring-collision shape: a *durable* appliance or utensil named after the food it acts on.** ICA's back-to-school/summer week put "Kaffebryggare" in `kaffe_te`, "Brödrost" in `matbrod`, and "Frukt- och svamptork" in `frukt`. `NON_FOOD_GATE_RE` couldn't help — these aren't hygiene/cleaning consumables, they're hardware, and the taxonomy has no leaf for hardware, so `ovrigt` is the honest answer. Added `NON_FOOD_DURABLE_RE` (checked *before* `BRAND_OVERRIDES`, since a brand fact would otherwise preempt it), kept deliberately narrow — specific head-nouns, not "anything electrical": bare `tork` would eat "torkad frukt", so only the attested compounds (`svamptork`/`frukttork`) are listed, and `panna`/`gryta` are excluded outright since both name real dishes. The same week produced three more collisions of already-documented shapes, all fixed the same targeted way: `räk(?!na)` — "räkna"/"Räkna med Greta" was landing in `skaldjur`, and "räkna" is an extremely common Swedish word; `(?<!s)kola(?!dricka)` — "s*kola*n" (school) was landing in `godis`; and `\biste\b` — "iste" (iced tea) was unanchored while its sibling `te` already was, so it matched inside "Kl*iste*rmärkesbok" and "Tw*iste*r". Note `\b` genuinely works for `iste` (pure ASCII) even though it famously does *not* for `öl`/`Grevé` — the difference is whether an accented letter sits at the boundary.
- **The "blanket brand override" trap has a detector now — use it.** `erbjudanden-verify.mjs`'s cross-source consistency check caught "Kattsand | SMART PETS" → `djurmat` while "Kattsand | BEST FRIEND" → `djurgodis_tillbehor`, purely because `smart pets` was a blanket `BRAND_OVERRIDES` entry preempting the far more specific "kattsand" in the product's own name. Smart Pets sells food, treats *and* litter — exactly the "brand spans product types" condition under which the existing Dafgårds/Billys lesson says a bare brand name must **not** be hardcoded. Removed the brand entry and added `kattgodis|hundgodis|djurgodis` as real product keywords instead (without which "Kattgodis" falls through to `SNACKS_RE` and becomes human candy). When a verify WARN says one product name got two categories *within the same import*, suspect a brand override before suspecting the keyword lists.
- **ICA's non-food share is seasonal, and the drop list is the reviewer's job, not the parser's.** The W33 ICA webarchive carried 252 offers of which **169 (67%) were non-grocery** — back-to-school stationery/books, summer clearance toys, kitchen appliances, clothing — vs. roughly half in the W29 import. The keep rule that has held across weeks: food plus household consumables restocked on a normal grocery run (cleaning, hygiene, paper/bags, pet supplies, baby care); drop durables (appliances, cookware, toys, clothing, books, stationery, garden, sport, baby gear). Filtering on `kategori === 'ovrigt'` alone is **not** sufficient — non-food that keyword-matched a *food* category (a crayfish-party decor kit in `skaldjur`, an electric toothbrush in `hygien`, storage tubs in `fil_yoghurt`) survives that filter and needs an explicit by-name drop list.
- **A parser keyed to span *position* fails silently too — not just one keyed to a hash class.** `erbjudanden-parse-ica-html.mjs` read `offer-card__text`'s spans as `spans[0] = brand/size, spans[1] = Jmfpris/Ord.pris/30dgr`. A card whose `data-promotion-brand` is empty renders the bold span as a bare `<!---->` placeholder, so the *detail* span lands at index 0 — 55 of 306 offers in the 2026-W35 import came out with `marke: "Ord"` and `storlek: "pris 179:00 kr, ..."`, and lost their real `ord_pris`/`pris_30dgr`/`jamforpris` (ord_pris coverage 246→301 once fixed). This is *worse* than the W33 Hemköp hash-class break, which at least announced itself as a suspicious `0`: here the offer count was perfectly correct and only the field contents were garbage. Fixed by picking each span by its own `offer-card__text--bold` class instead of its index. **Generalized check for every import**: don't only compare the offer *count* against last week — spot-check that `marke`/`ord_pris` still look like a brand and a price. Same file also now drops a punctuation-only brand segment (ICA ships a literal `",. "` bold block on some brandless cards, which became `marke: ","`).
- **A "+pant" suffix on an ICA price splash silently dropped the offer entirely** (2026-W36): `erbjudanden-parse-ica-html.mjs`'s `parseSplash` knew `"29,90 kr/+pant"` (slash form) but not `"29,90 kr +pant"` / `"3 för 42 kr +pant"` (space form) — an unparseable splash makes the parser treat the whole card as "not an offer", so Juice and Energidryck vanished with no error, only a slightly-low offer count. Fixed by stripping an optional trailing `+pant` in either spelling *before* the shape patterns and re-appending it to `pris_text`. Generalized check for every import: don't only compare this week's count against last week's — diff the parsed product **names** against the source HTML's own name attributes (`data-promotion-name` for ICA, `data-testid="product-title"` for Hemköp, `itemprop="name"` for Willys) and look at every card the parser skipped. That diff is what surfaced both this bug and the two Hemköp coupons below; a raw count alone would not have.
- **An ambiguous product can be settled by looking at its own product photo** (2026-W36): Willys/Hemköp cards carry an Axfood asset URL per product (`https://assets.axfood.se/image/upload/f_auto,t_200/<EAN>_C1x1_sNN`), so `curl` + the `Read` tool identifies a name-only product in two cheap calls — this is how "Fullkornsgott | Garant" (a fullkorn *loaf*, not cereal), "Gårdsgoda | Valio" (a smör/raps *spread*, misfiled `ost_hard`), "Cinelle | Dafgårds" (a *wienerbröd*, misfiled `mjolk_gradde`) and "Familjefavoriter | ARLA" (Gouda — the existing `ost_hard` verdict, confirmed rather than assumed) were each resolved. Two gotchas: only the `t_200` transform the page itself uses returns 200 (`t_400` gives HTTP 400 with a zero-byte body), and the URL must be pulled from **inside that product's own card block** — a proximity/window regex over the surrounding HTML grabs a neighbouring product's image and looks perfectly plausible while being the wrong product.
- **Six more substring/plural collisions, again found by eye-reading the week's own categorised output** (2026-W36), all fixed in `kategoriClassify.mjs`: `Hälsokost | BIOSALMA` → `ost_hard` (a supplement, and the *third* distinct "…ost" collision after `rostbiff`/`Costa Rica` — fixed as a real `halsa` leaf rule + gate word, not another haystack strip); `Verum filmjölk` → `mjolk_gradde` (soured milk is `fil_yoghurt`, and the same corpus already held the correct verdict for the sibling product); `Valnötter | EXOTIC SNACKS` → `chips_snacks` because `\bvalnöt\b` never matched the plural and the brand's own word "snacks" won instead; `Vegetariska korvar` → `ovrigt` because `korv\b` demanded a boundary the plural "korvar" doesn't have (a vego sausage must land in `korv` per this taxonomy's own vego rule); `Ekologiska pumpakärnor` → `gronsaker` via `VEG_GENERIC_RE`'s "pumpa" (seeds now live in `notter_torkat` alongside `linfrö`/`chiafrö`); and bare standalone `Kakor | FILIPINOS` matching nothing at all (added as `(?:^| )kakor(?: |,|$)` **only** as a standalone word, so `riskakor`→`knackebrod`, `chokladkakor`→`choklad` and `pannkakor`→`enportionsratt` keep their own better verdicts — the plural trap from W35 cuts both ways). Also `picanha` was moved out of the `charkuterier` pattern into `PROTEIN_GENERIC_RE`: it's a raw beef cut, not charcuterie. Same standing rule as W35 — a classifier fix does **not** re-derive cached `regel` lexicon verdicts, so the stale keys have to be deleted (only where the stored kategori still equals the *wrong* value, never blanket) and re-derived; `kalla: 'manuell'` entries are the right home for the handful the rules genuinely can't know.
- **Hemköp's "Bara för dig" coupons can be last week's leftovers** (2026-W36): two personal 10%-coupons carried `end-date` = the *first* day of the new week's window while all 57 regular offers ran the full Mon–Sun. They also render in a card shape the parser doesn't handle, so they were left out and named in `urval` rather than hand-added — but check the per-item `data-testid="end-date"` distribution on every Hemköp import: if a large share (not just a couple of coupons) ends on day one, the capture is of the *outgoing* week, which the file-level `giltigt_till` would otherwise silently misstate.
- **Five more substring collisions, all found by reading the week's own categorised output by eye** (2026-W35) — the spot-check step the skill's §3.2 asks for, and the only thing that catches a *confident* wrong verdict: `Hela bönor | LAVAZZA` → `baljvaxter` (the existing "kaffe AND böna" check needs the word "kaffe", and here the coffee-ness is only in the brand — fixed with a `lavazza` `BRAND_OVERRIDES` entry, safe because Lavazza makes nothing but coffee); `Majskakor | FRIGGS` → `gronsaker`, because **`BROD_RE`'s `kaka(?!o)` never matched the Swedish plural `kakor`** so the bread gate lost to `VEG_RE`'s "majs" (same plural-shift trap as `morot`/`morötter`, worth grepping for wherever a singular keyword is used); `Babyplommontomater` → `frukt` via "plommon" (rewrite `plommontomat*` → `tomat`); `Ostkrokar` → `ost_hard`, a spelling the existing `ostbågar|ostsnacks` → `snacks` rewrite didn't cover; `Ansiktsrengöring | GARNIER` → `stad_disk_tvatt` via "rengör" (fixed with `(?<!ansikts)rengör`, the same shape as the earlier `fluorskölj` fix on that very rule). Plus one plain leaf-list gap: `Smältost` (processed spreadable cheese) had no pattern at all and fell to `ost_hard`. All six fixed in `kategoriClassify.mjs` with the golden set still green; the lexicon entries had to be corrected separately, since a cached `regel` verdict is never re-derived by a classifier change.
