...
**Aisle ("store walk") ordering (2026-07)**: the list is meant to end up ordered by which store an item should be bought in, then by that store's actual shelf layout — but neither shopping-list items nor recipes carry a store assignment yet, so `src/lib/storeOrder.ts` ships an interim single-store "normal order" (`AISLE_ORDER`: Frukt & Grönt → Bröd → Kött & Fisk → Mejeri & Ägg → Skafferi → Fryst → Dryck → Snacks & Godis → Hushåll & Hygien → Övrigt) as a placeholder until real per-store aisle layouts are configured. `guessAisleCategory(name)` keyword-matches an item's `vara` string into one of those buckets (unmatched falls to `ovrigt`, same catch-all convention as the `erbjudanden` category taxonomy); `sortByAisle`/`groupByAisle` sort/group by that bucket then alphabetically. `aggregateIngredients` (`shoppingList.ts`) sorts its output this way now instead of plain alphabetical, and `HandlaView` groups the Ingredienser, bevaka-hits, and Eget-tillägg lists into aisle sections (`.shop-aisle-header`) the same way, so all three sources read in one consistent walk order. **Watch for the substring-collision trap already documented below** when extending the keyword lists — this categorizer hit it three times in one pass: `färs` (mince) inside `färskt` (fresh), `mjöl` (flour) inside `mjölk` (milk), and `fil` (cultured milk) inside `filé` (fillet, e.g. "Laxfilé") — all fixed with a stripped/negative-lookahead guard rather than trusting the raw substring match. When real per-store aisle configs eventually land, they'll need each item to also carry (or resolve to) a specific store — this module only orders *within* one generic layout, it doesn't yet decide *which* store.

**"Show one store at a time" (2026-07)**: a `.handla-storebar` pill row ("Alla"/Willys/ICA/Hemköp, reusing `FyndView`'s `.fynd-chip` styling and `STORES` from `bevaka.ts`) lets the shopper filter `HandlaView` down to one store while actually standing in it. What gets filtered depends on whether we *know* the item's store: bevaka hits carry a real store on their matched offer, so `hitsForStore` (`bevaka.ts`) narrows each hit's `offers` to just that store's and drops hits with no match there — no tagging needed, this is exact. Recipe ingredients have no store concept in the data model at all (a carrot isn't "a Willys carrot"), so they deliberately stay visible in every store tab — filtering them out would just hide things you still need to buy. Manual items sit in between: usually store-agnostic like ingredients, but occasionally added *because* of a specific store ("toilet paper, the big Willys pack") — so they get an optional per-item tag (`useShoppingList`'s new `storeAssignments: Record<id, storeKey>` field, additive on the existing `matracet:shopping:v1` key rather than a version bump, since wiping `removedIds`/`manualItems` to add one field would lose real usage data) set via a small pill button on the row that cycles willys → ica → hemkop → unset (`cycleStore`). Untagged manual items show in every store tab, same as ingredients. **CSS trap hit while building the store bar**: originally nested `.handla-storebar` inside `.handla-grid` as a `grid-column: 1 / -1` item spanning both columns — Chrome's grid auto-row-sizing inflated that row to ~127px (vs. the row's actual ~45px content+margin) for reasons not fully root-caused; moved the store bar to its own `.handla-storebar-wrap` sibling *outside* the grid (between `TopBar` and `.screen-body.handla-grid`, own small padding wrapper) and the phantom gap disappeared immediately. If a future full-width "spans both grid columns" element in this view shows unexplained extra vertical space, try pulling it out of the grid before spending more time on the CSS.
